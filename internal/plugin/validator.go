...
func (v *Validator) Validate(plugin interface{}) bool {
	// Step 1: 检查是否实现MiddlewarePlugin接口
	_, ok := plugin.(MiddlewarePlugin)
	if !ok {
		log.Println("插件未实现MiddlewarePlugin接口")
		return false
	}

	// Step 2: 验证插件元数据
	p := plugin.(MiddlewarePlugin)
	if p.Name() == "" {
		log.Println("插件名称不能为空")
		return false
	}

	// SupportedTypes is gone, check Type
	if p.Type() == "" {
		log.Println("插件必须声明类型")
		return false
	}
...
func (v *Validator) checkSecurity(plugin MiddlewarePlugin) error {

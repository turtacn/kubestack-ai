...
**Usage**:
```bash
ksa diagnose -t redis --instance localhost:6379 -o json
```
...
**Usage**:
```bash
ksa diagnose -t [middleware-type] [flags]
```
...
**Usage**:
```bash
ksa diagnose -t redis --instance my-redis --namespace production
```
...
**Usage**:
```bash
ksa diagnose -t redis --instance localhost:6379 --async
```
...
ksa diagnose -t mysql --instance db-01 --namespace prod

# Async diagnosis
ksa diagnose -t kafka --instance localhost:9092 --async

# Dry run
ksa diagnose -t postgresql --instance localhost:5432 --dry-run
```
...
**Usage**:
```bash
ksa diagnose -t redis --instance localhost:6379 -o text
```
...
**Usage**:
```bash
ksa diagnose -t redis --instance localhost:6379 -o yaml
```
...
ksa diagnose -t redis --instance localhost:6379 -o json > diagnosis.json
...
for instance in redis-01 redis-02 redis-03; do
  ksa diagnose -t redis --instance $instance:6379 -o json > diag-$instance.json
done
```
...
#!/bin/bash
# Health check script
RESULT=$(ksa diagnose -t redis --instance $REDIS_HOST -o json)
...
chmod +x ksa
# OR
sudo ksa diagnose -t redis --instance localhost:6379
```
